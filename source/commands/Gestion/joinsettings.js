...
module.exports = {
    name: 'joinsettings',
    description: 'Permet de configurer le syteme de joins du bot',
...
if (pass === false) {
    if (client.noperm && client.noperm.trim() !== '') {
        ...
        async function updateEmbed() {
            ...
            const channels = channel ? `${channel.name} - ID ${db.channel || 'Inconnue'}` : 'Inconnue';
            const messages = db?.message || 'Aucun message';
        ...
        collector.on("collect", async (i) => {
            ...
            if(i.values[0] === `status_${message.id}`) {
                ...
                const salonrow = new Discord.ActionRowBuilder().addComponents(
                    ...
                    .setChannelTypes(0)
                    .setMinValues(1)
                        .setCustomId('joinsettings_setup_salon_' + message.id)
                )
        ...
        client.on('interactionCreate', async (i) => {
            if (message.author.id === i.user.id) {
                ...
                if (i.customId === `joinsettings_setup_salon_${message.id}`) {
